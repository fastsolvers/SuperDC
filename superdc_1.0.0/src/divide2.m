%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%% SuperDC dividing stage (new, stable) %%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


function [D, B, Z, desc, nflops] = divide2(D, U, B, R, tr)
%%% Input:
...
for i = n:-1:1
    ...
    if isempty(ch{c1})
        if size(B{c1}, 1) <= size(B{c1}, 2)
            ...
            D{c1} = D{c1} - (T * T') / norm(B{c1});
            nflops = nflops + flops('prod', T, 'n', T, 't') + 2 * flops('sum', D{c1});
        end
    else
        if size(B{c1}, 1) <= size(B{c1}, 2)
            Sp = sqrt(norm(B{c1})) * eye(size(R{ch{c1}(1)}, 2)); 
        else
            Sp = B{c1} / sqrt(norm(B{c1}));
        end
        S = {};
        S = push(S, Sp);                   
    ...
    if isempty(ch{c2})
        ...
        if size(B{c1}, 1) <= size(B{c1}, 2)
            ...
            Sp = sqrt(norm(B{c1})) * eye(size(R{ch{c2}(1)}, 2)); 
        end
        S = {};
        S = push(S, Sp);
